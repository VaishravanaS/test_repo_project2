L 1 "../src/ADS1298Init.c"
N /******************************************************************************
N**File Name			: ADS1298Init.c
N**File Description	:
N**Created By		:
N**Creation Date		:
...
N			 0x0000,
N		    //LOFF_SENSP
N		     0x0006,//enabled only for 3 input channels
N		    //LOFF_SENSM
N		     0x0006,
N			//LOFF_FLIP (default)
N			 0x0000,
...
N			//0x00E6,
N			 //CONFIG4
N		     0x0022,//single -shot conversion mode enabled
N			 //0022 respiration modulation enabled with frequency 32kHz
N			 //0x0002,//64khz
N			 //WCT1
N		     0x0000,
N		    //WCT2
N		     0x0000,
N//changed WCT1 WCT2 RLDSENSP and RLDSENSN config3
N
N};
N
N
...
N
N/*--------------------------------------------------------------------------------------------**
N** 	Function Name: ECG_ADS1298_INIT                                                           **
N** 	Description : 	- The function initializes the ADS1298 registers with the                 **
N** 					  required values. ADS1298 is configured as follows:-                     **
N** 					                                                                          **
N** 					  Set pin CLKSEL to high (GPIO11 of C5505 EVM to send high)               **
N** 					  CONFIG1 - 0xC2 for data rate = 8000 SPS PER Channel                     **
N** 					  CONFIG2 - 0xE0 for setting test source,test amplitude & test freq.      **
N** 					  CONFIG3 - 0xC0 for VREFP set to 2.4, internal power down ref buffer     **
N** 					  CONFIG4 - 0x02 for pulse mode selected; DC lead off turned on           **
N** 					  LOFF - 0x03 for lead off                                                **
N** 					  LOFF_SENSP -0xFF to turn on P-side of all channels for lead off sensing **                        
N** 					  LOFF_SENSM -0xFF turn on M-side of all channels for lead off sensing    **
N** 					  GPIO - 0x00 configuring as output pins as GPIO is not used.             **
N**                                                                                            **                   
N** 	Parameter : None						   		                                          **
...
N	 Uint16 comWLen = 0;
N	 Uint16 comFLen = 0;
N/*to power up the ECG module in the final board*/
N	 //*GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0400;
N
N/*to power down the ECG_module in the final board*/
N	 //*GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR & 0xFBFF;
N
N	 /*Reset ADS1298*/
N	 //Setting RESETB pin to high
N	 *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0002; //to be replaced by *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0800;
X	 *((ioport volatile unsigned*)0x1C0A) = *((ioport volatile unsigned*)0x1C0A) | 0x0002; 
N	 //Delay 
N	 wait(1000);
N
N     //Setting RESETB pin to low
N	 *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR & 0xFFFD; //to be replaced by *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR & 0xF7FF;
X	 *((ioport volatile unsigned*)0x1C0A) = *((ioport volatile unsigned*)0x1C0A) & 0xFFFD; 
N
N	 //Delay 
N	 wait(1000);
N
...
X	 *((ioport volatile unsigned*)0x1C0A) = *((ioport volatile unsigned*)0x1C0A) | 0x0002; 
N
N	 // Wait for power on reset. Delay approx >32msec
N     wait(74000);
N
N#if SW_CLK_SEL
X#if 1
N	 /*Selecting external clock*/
N
N	 *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0800;//
X	 *((ioport volatile unsigned*)0x1C0A) = *((ioport volatile unsigned*)0x1C0A) | 0x0800;
N
N	 wait(10000);
N	 //Setting CLKSEL of ADS1298 to low
N	 *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR & 0xF7FF;//CLKSEL not being used in final board, but used here for lcd power
X	 *((ioport volatile unsigned*)0x1C0A) = *((ioport volatile unsigned*)0x1C0A) & 0xF7FF;
N#endif    
N	 //delay for internal oscillator to start up
N	 wait(74000);
N
N	 /*Setting START pin to low*/
N	 *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR & 0xFFEF;//chip select to be asserted low
...
N	 wait(74000);
N
N	 //Setting START pin to high
N	 /*vai *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0010;*/    //not being used can be removed later
N	 /*
N	     *GPIO_DOUT0_ADDR = *GPIO_DOUT0_ADDR | 0x0010;
N	 */
N
N	 wait(100000);
N
...
N     Status = LLC_SPI_WordLengthWrite(ADS1298RegVal, wLen, fLen);	 
N     if(Status != PSP_SOK)
X     if(Status != (0))
N	 {
N        return (Status);
...
N
N    
N	 /* Register Read Command for 23 registers from address 01h*/
N	 /*Get ADS1298 registers Values back to verify*/
N     Command[0] = CMD_RREG1-1;
X     Command[0] = 0x21-1;
N#if  (NEW_PG_BOARD ==1)
X#if  (1 ==1)
N     Command[1] = CMD_RREG2+3;
X     Command[1] = 0x16+3;
N#else    
S     Command[1] = CMD_RREG2+1;
N#endif    
N	 comWLen = 8;
N	 comFLen = 2;
N	 wLen = 8;
...
N	SPIBuf[0] = 0;
N	//
N	Status = LLC_SPI_WordLengthWriteRead(SPIBuf, Command, wLen, fLen, comWLen, comFLen); 
N	 if(Status != PSP_SOK)
X	 if(Status != (0))
N	 {
N        return (Status);
