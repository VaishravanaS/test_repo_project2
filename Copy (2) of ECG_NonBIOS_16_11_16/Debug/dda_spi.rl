...
N#endif /* _DDA_SPI_H_ */
N/*EOF*/
L 14 "../src/dda_spi.c" 2
N
NDDA_spiConfig   spiConfig;
N
NUint16          spiSlavesOpen;
N/**< no of slaves open                                              */
N
Nextern SPI_Handle  *pSpiInst;
Nextern Uint16 LLC_SPI_WLenGet(void);
N/**< pointer spi handle                                             */
N
N/**
N *  \brief  enable the SPI module
N *
...
N    Uint16 delay;
N
N	CSL_FINS((*IDLE_PCGCRL_ADDR), IDLE_PCGCRL_SPI_IDLE, 0);
X	(((*((ioport volatile unsigned*)0x1C02))) = (((*((ioport volatile unsigned*)0x1C02))) & ~(0x0002u)) | (((0) << (0x0001u)) & (0x0002u)));
N
N	/* Value of 'Reset Counter' */
N	CSL_FINS((*PER_RSTCOUNT_ADDR), PER_RSTCOUNT, 4);
X	(((*((ioport volatile unsigned*)0x1C04))) = (((*((ioport volatile unsigned*)0x1C04))) & ~(0xFFFFu)) | (((4) << (0x0000u)) & (0xFFFFu)));
N	CSL_FINS((*PER_RESET_ADDR), PER_RESET_RESETEN_LCD_IIS2_IIS3_UART_SPI, 1);
X	(((*((ioport volatile unsigned*)0x1C05))) = (((*((ioport volatile unsigned*)0x1C05))) & ~(0x0080u)) | (((1) << (0x0007u)) & (0x0080u)));
N
N	for(delay = 0; delay < 100; delay++);
N
N	CSL_FINS((*PERIPHSEL0_ADDR),PERIPHSEL0_SEL_PARALLELPORT, 0x5);
X	(((*((ioport volatile unsigned*)0x1C00))) = (((*((ioport volatile unsigned*)0x1C00))) & ~(0x7000u)) | (((0x5) << (0x000Cu)) & (0x7000u)));
N	/*change 0x5 to 0x001 in the final board*/
N}
N
N/**
N *  \brief Initialize SPI controller
N *
N *  \param  spiConfig   [IN]   spi config structure
N *  \param  spiClkRate      [IN]    Clock divisor value
N *  \param  frameLen    [IN]    Frame length
N *  \param  wordLen     [IN]    Word length
N *  \param  slaveNo     [IN]    slave number to transact
N *  \param  dataDly     [IN]    Data delay
N *  \param  csPol       [IN]    CS polarity
N *  \param  clkPh       [IN]    Clock Phase
N *  \param  clkPol      [IN]    Clock Polarity
N *
N *  \return status of init
N */
NPSP_Result SPI_INIT(
...
N
N    result = DDA_spiConfiguration(spiConfig.spiCtrlMode,
N                                  spiConfig.spiMaster,
N                                  spiConfig.opMode,
N                                  spiConfig.instances,
N                                  spiConfig.syncMode);
N    if (PSP_SOK != result)
X    if ((0) != result)
N    {
N        return (result);
N    }
N   /* Initialization required for enable SPI module*/
N	spiPreInit();
N
N    pSpiInst = NULL;
X    pSpiInst = 0;
N
N    result = DDC_SPI_Init(spiClkRate, frameLen, wordLen);
N    if (result != PSP_SOK)
X    if (result != (0))
N    {
N        return(result);
...
N    spiSlavesOpen = 0;
N
N    /* chip select and slave configurations */
N    result = DDA_spiSlaveSelect((SPI_SlaveSel)slaveNo,
N                                (SPI_SlaveClkPh)clkPh,
N                                (SPI_SlaveClkPolarity)clkPol,
N                                (SPI_SlaveDataDly)dataDly,
N                                (SPI_SlaveSelPol)csPol);
N    if(PSP_SOK != result)
X    if((0) != result)
N    {
...
N                              Uint16 readOrWrite)
N{
N    Uint16 wordLength = 0;
N    PSP_Result result = PSP_E_INVAL_PARAM;
X    PSP_Result result = (-5);
N    PSP_Result transferStatus = PSP_E_INVAL_PARAM;
X    PSP_Result transferStatus = (-5);
N
N    if ((count > 0) && (buffer == NULL))
X    if ((count > 0) && (buffer == 0))
N        return (result);
N
N    if ((cmdCount > 0) && (cmdBuf == NULL))
X    if ((cmdCount > 0) && (cmdBuf == 0))
N        return (result);
N
N    wordLength = LLC_SPI_WLenGet();
N    if (wordLength == 8)
N    {
N        result = (PSP_Result) LLC_SPI_FLenSet(count+cmdCount);
N        if (result != PSP_SOK)
X        if (result != (0))
N            return (result);
N    }
N    else if(wordLength == 16)
N    {
N        result = (PSP_Result) LLC_SPI_FLenSet(count+cmdCount);
N        if (result != PSP_SOK)
X        if (result != (0))
...
N    if(cmdCount != 0)
N    {
N        transferStatus = (PSP_Result) spiWriteCmd(cmdBuf,cmdCount,wordLength,readOrWrite);
N        if(transferStatus != PSP_SOK)
X        if(transferStatus != (0))
N            return (transferStatus);
N    }
N
N    if(count != 0)
N    {
N       if(readOrWrite == SPI_READ)
N       {
N	      transferStatus = (PSP_Result) spiReadData(buffer,count,wordLength);
N       }
N       else if (readOrWrite == SPI_WRITE)
N       {
N           transferStatus = (PSP_Result) spiWriteData(buffer,count,wordLength);
N       }
N       else
N       {
...
N *  \return status of write
N */
NPSP_Result SPI_WRITE_BYTES(
N                       Uint16 data)
N{
N    PSP_Result writeStatus;
N
N    writeStatus = PSP_E_DRIVER_INIT;
X    writeStatus = (-1);
N
N    writeStatus = spiWriteData(&data, SPI_WRITE_1_WORD, 16);
X    writeStatus = spiWriteData(&data, (0x01), 16);
N
N    return writeStatus;
N}
N
N/**
N *  \brief  spi driver configurations
...
N                        DDA_SPI_SYNC_ASYNC      syncMode)
N{
N    if( (SPI_MODE        == spiCtrlMode) &&
N        (SPI_MASTER      == spiMaster)   &&
N        (SPI_POLLED      == opMode)      &&
N        (SPI_SINGLE_INST == instance)    &&
N        (SPI_SYNC        == syncMode))
N    {
N        spiConfig.spiCtrlMode = spiCtrlMode;
N        spiConfig.spiMaster   = spiMaster;
N        spiConfig.opMode      = opMode;
N        spiConfig.instances   = instance;
N        spiConfig.syncMode    = syncMode;
N        return (PSP_SOK);
X        return ((0));
N    }
N    else
...
N                        )
N{
N    SPI_SlaveConfig slaveConfig;
N    SPI_HwMode mode;
N
N    slaveConfig.slaveNo = slaveNo;
N    slaveConfig.slvClkPhase = slvClkPhase;
N    slaveConfig.slvClkPolr = slvClkPolr;
N    slaveConfig.slvDataDly = slvDataDly;
N    slaveConfig.csPolr = csPolr;
N
N    /*
N     * check if a slave is already open and communicating over SPI.
N     * If so another slave cannot be opened, return failure
N     */
N    if(spiSlavesOpen > SPI_OPEN_INSTANCES_MAX)
X    if(spiSlavesOpen > (1))
N        return (PSP_E_INVAL_PARAM);
X        return ((-5));
N
N
N    if((slvClkPolr ==0)&&(slvClkPhase ==0))
N    {
N        mode = SPI_MODE_0;
N    }
N    else if((slvClkPolr ==0)&&(slvClkPhase ==1))
N    {
N        mode = SPI_MODE_1;
N    }
N    else if ((slvClkPolr ==1)&&(slvClkPhase ==1))
N    {
N        mode = SPI_MODE_2;
N    }
N    else 
...
N   
N    
N    pSpiInst = DDC_SPI_Open(mode, slaveConfig);
N    if (pSpiInst != NULL)
X    if (pSpiInst != 0)
N    {
N        spiSlavesOpen++;
...
N{
N    /*
N     * slave deselect is only allowed if already a slave is present
N     * if a slave is not already selected, return failure
N     */
N    if (spiSlavesOpen <= 0)
N    {
N        return (PSP_E_INVAL_PARAM);
X        return ((-5));
N    }
N
N    DDC_SPI_Close(pSpiInst);
N    if(NULL == pSpiInst)
X    if(0 == pSpiInst)
N    {
N        spiSlavesOpen--;
...
N    Uint16 value = 0;
N
N    do { /* Check for bsy status */
N            value=CSL_FEXT(CSL_SPI_REGS->SPISR1,SPI_SPISR1_BSY);
X            value=(((((CSL_SpiRegsOvly) 0x3000)->SPISR1) & (0x0001u)) >> (0x0000u));
N        }while(value & 0x01==1);
N
N
N    CSL_FINS(CSL_SPI_REGS->SPICR1,SPI_SPICR1_FLEN,1);       /* set one word */
X    ((((CSL_SpiRegsOvly) 0x3000)->SPICR1) = ((((CSL_SpiRegsOvly) 0x3000)->SPICR1) & ~(0x0FFFu)) | (((1) << (0x0000u)) & (0x0FFFu)));        
N    /* Send Read Status Command */
N    CSL_FINS(CSL_SPI_REGS->SPIDR2,SPI_SPIDR2_DATA,0x0500);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) & ~(0xFFFFu)) | (((0x0500) << (0x0000u)) & (0xFFFFu)));
N    CSL_FINS(CSL_SPI_REGS->SPIDR1,SPI_SPIDR1_DATA,0x0);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPIDR1) = ((((CSL_SpiRegsOvly) 0x3000)->SPIDR1) & ~(0xFFFFu)) | (((0x0) << (0x0000u)) & (0xFFFFu)));
N
N    /* set start CMD - Read */
N    CSL_FINS(CSL_SPI_REGS->SPICR2,SPI_SPICR2_CMD,0x01);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) & ~(0x0003u)) | (((0x01) << (0x0000u)) & (0x0003u)));
N
N    do { /* Check for Word Complete status */
N        value=CSL_FEXT(CSL_SPI_REGS->SPISR1,SPI_SPISR1_WC);
X        value=(((((CSL_SpiRegsOvly) 0x3000)->SPISR1) & (0x0002u)) >> (0x0001u));
N    }while(value & 0x02==0);
N
N    do { /* Check for bsy status */
N        value=CSL_FEXT(CSL_SPI_REGS->SPISR1,SPI_SPISR1_BSY);
X        value=(((((CSL_SpiRegsOvly) 0x3000)->SPISR1) & (0x0001u)) >> (0x0000u));
N    }while(value & 0x01==1);
N
N    //Read the Status --- Dummy clock cycles
N    CSL_FINS(CSL_SPI_REGS->SPIDR2,SPI_SPIDR2_DATA,0x0);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) & ~(0xFFFFu)) | (((0x0) << (0x0000u)) & (0xFFFFu)));
N    CSL_FINS(CSL_SPI_REGS->SPICR2,SPI_SPICR2_CMD,0x01);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) & ~(0x0003u)) | (((0x01) << (0x0000u)) & (0x0003u)));
N
N    do { /* Check for Word Complete status */
...
N    }while(value & 0x01==1);
N
N    // Enable write using WREN command
N    CSL_FINS(CSL_SPI_REGS->SPICR1,SPI_SPICR1_FLEN,0x0);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPICR1) = ((((CSL_SpiRegsOvly) 0x3000)->SPICR1) & ~(0x0FFFu)) | (((0x0) << (0x0000u)) & (0x0FFFu)));
N
N    /* Write Enable command */
N    CSL_FINS(CSL_SPI_REGS->SPIDR2,SPI_SPIDR2_DATA,0x0600);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPIDR2) & ~(0xFFFFu)) | (((0x0600) << (0x0000u)) & (0xFFFFu)));
N    CSL_FINS(CSL_SPI_REGS->SPIDR1,SPI_SPIDR1_DATA,0x0000);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPIDR1) = ((((CSL_SpiRegsOvly) 0x3000)->SPIDR1) & ~(0xFFFFu)) | (((0x0000) << (0x0000u)) & (0xFFFFu)));
N
N    /* set start CMD - Write */
N    CSL_FINS(CSL_SPI_REGS->SPICR2,SPI_SPICR2_CMD,0x02);
X    ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) = ((((CSL_SpiRegsOvly) 0x3000)->SPICR2) & ~(0x0003u)) | (((0x02) << (0x0000u)) & (0x0003u)));
N
N    do { /* Check for Word Complete status */
N        value=CSL_FEXT(CSL_SPI_REGS->SPISR1,SPI_SPISR1_WC);
X        value=(((((CSL_SpiRegsOvly) 0x3000)->SPISR1) & (0x0002u)) >> (0x0001u));
