...
L 12 "../src/SPO2_functions.c" 2
N#define SPO2_AVERAGE_LENGTH 10
N#define ACCURACY 10
N#define FILTER_LENGTH 5
N#define BUFFER 6
N
N#define DEBUG
N#ifdef DEBUG
N//#define PRINT_RED_IR_RAW_DATA
N//#define PRINT_RED_IR_FILTERED_DATA
N//#define PRINT_PEAKS
N//#define PRINT_DC
N//#define STORE_PEAKS
N//#define PRINT_RATIOS
N#define PRINT_AVERAGE_SPO2
N#define CORRECT_DC
N#endif
N
N
N
Nunsigned long adc_value,adc_value1;
Nunsigned long raw_RED_buffer[FILTER_LENGTH_SPO2];
Xunsigned long raw_RED_buffer[5];
Nfloat SPO2;
Nunsigned long raw_IR_buffer[FILTER_LENGTH_SPO2];
Xunsigned long raw_IR_buffer[5];
N
N
Nextern unsigned char SPO2_Percentage;
Nextern Uint8 SystemState_ProcessingSPO2dataStatus;
Xextern unsigned char SystemState_ProcessingSPO2dataStatus;
N
N
N
N
...
N	unsigned int j;
N	double sum=0;
N	for (j=0; j <SPO2_AVERAGE_LENGTH;j++)
X	for (j=0; j <10;j++)
N	{
N		sum+=u16_Raw_Data[j];
N	}
N	avg =sum/SPO2_AVERAGE_LENGTH;
X	avg =sum/10;
N	return avg;
N}
N
Nunsigned long calculate_ratio(unsigned long dc,unsigned long peak,unsigned long valley)
N{
N	unsigned long  rat=0;
N	//rat=(peak-dc)/(two_root_two*dc);
N	rat=(unsigned long )(peak-valley)*10000;
N	rat=rat/(dc);
N	return rat;
N}
Nfloat calulate_AC_to_DC_Ratio(unsigned long a,unsigned long b )
N{	unsigned int k;
N	static unsigned long present_adc_value[BUFFER],present_adc_value1[BUFFER];
X	static unsigned long present_adc_value[6],present_adc_value1[6];
N	static float R,SPO2_buffer[SPO2_AVERAGE_LENGTH],SPO2_avg;
X	static float R,SPO2_buffer[10],SPO2_avg;
N	static unsigned long pos_avg_peak1,neg_avg_peak1,pos_peak1,neg_peak1,temp_peak1;
N	static unsigned long pos_avg_peak,neg_avg_peak,pos_peak,neg_peak,temp_peak;
N	static unsigned int present_direction_flag1,previous_direction_flag1,temp_pos_direction_count1,temp_neg_direction_count1;
N	static unsigned int present_direction_flag,previous_direction_flag,temp_pos_direction_count,temp_neg_direction_count;
N	static long long sum_dc1,old_sum1,new_sum1;
N	static long long sum_dc,old_sum,new_sum;
N	static unsigned int dc_count,dc_count1;
N	static unsigned long average_dc_value,average_dc_value1;
N	static unsigned long  ratio,ratio1;
N	static unsigned long red_adc_buffer[ACCURACY],ir_adc_buffer[ACCURACY];
X	static unsigned long red_adc_buffer[10],ir_adc_buffer[10];
N	present_adc_value[0]=a;
N	present_adc_value1[0]=b;
N
N	/*for calculating dc;summing every sample*/
N	sum_dc=sum_dc+present_adc_value[0];
N	dc_count++;
N	sum_dc1=sum_dc1+present_adc_value1[0];
N	dc_count1++;
N
N	#ifdef CORRECT_DC
N		for(k=0;k<(ACCURACY-1);k++)
X		for(k=0;k<(10-1);k++)
N		{
N			red_adc_buffer[k]=red_adc_buffer[k+1];
N			ir_adc_buffer[k]=ir_adc_buffer[k+1];
N		}
N		red_adc_buffer[ACCURACY-1]=present_adc_value[0];
X		red_adc_buffer[10-1]=present_adc_value[0];
N		ir_adc_buffer[ACCURACY-1]=present_adc_value1[0];
X		ir_adc_buffer[10-1]=present_adc_value1[0];
N		new_sum=0;
N		new_sum1=0;
N		for(k=0;k<(ACCURACY);k++)
X		for(k=0;k<(10);k++)
N		{
N			new_sum+=red_adc_buffer[k];
N			new_sum1+=ir_adc_buffer[k];
N		}
N	#endif
...
N
N
N			SystemState_ProcessingSPO2dataStatus=SystemState_ProcessingSPO2dataStates_valleydetected;
N
N
N			/*dc calculation*/
N			#ifdef CORRECT_DC
N			sum_dc=sum_dc+old_sum-new_sum;
N			old_sum=new_sum;
N			#endif
N
N			average_dc_value=sum_dc/dc_count;
N			sum_dc=0;
N			dc_count=0;
N			#ifndef CORRECT_DC
S			new_sum=0;
...
N
N			#ifdef PRINT_DC
S			printf("average_dc_value=%lu\r\n",average_dc_value);
N			#endif
N
N			/*ratio calculation*/
N			ratio=calculate_ratio(average_dc_value,pos_avg_peak,neg_avg_peak);
N			#ifdef PRINT_RATIOS
S			printf("ratio=%lu\n",ratio);
N			#endif
N		}
N		else
...
N			#endif
N			SystemState_ProcessingSPO2dataStatus=SystemState_ProcessingSPO2dataStates_peakdetected;
N			/*SPO2 calculation using the curve   110-25R  where R=ac_to_dc_ratio_RED/ac_to_dc_ratio_IR     */
N			for(k=0;k<(SPO2_AVERAGE_LENGTH);k++)
X			for(k=0;k<(10);k++)
N			{
N				SPO2_buffer[k]=SPO2_buffer[k+1];
N			}
N
N			R=((float)ratio/ratio1);
N			if(R>0&&R<2.024)
N			SPO2_buffer[SPO2_AVERAGE_LENGTH-1]=-30.363*R*R+11.962*R+100.34;
X			SPO2_buffer[10-1]=-30.363*R*R+11.962*R+100.34;
N			//SPO2_buffer[SPO2_AVERAGE_LENGTH-1]=110-25*R;
N			SPO2_avg=u32_Window_Average(SPO2_buffer);
N
N			//update SPO2 global variable
N			SPO2_Percentage=(unsigned char)SPO2_avg;
N			#ifdef PRINT_SPO2
S			printf("\t\t\t%.1f\r\n",SPO2_buffer[SPO2_AVERAGE_LENGTH-1]);
N			#endif
N			#ifdef PRINT_AVERAGE_SPO2
N//VS 9/8/16			printf("\t\t\t%.1f\r\n",SPO2_avg);
N			#endif
N		}
N
N	}
N
N	if(temp_peak1<present_adc_value1[0])
N	{
N		if(temp_pos_direction_count1==0)
N		{
N
N			neg_peak1=present_adc_value1[1];
N			for (k=2;k<BUFFER;k++)
X			for (k=2;k<6;k++)
N			{
N				if(present_adc_value1[k]<neg_peak1)
N				{
N					neg_peak1=present_adc_value1[k];
N				}
N			}
N		}
N		temp_pos_direction_count1++;
N		temp_neg_direction_count1=0;
N		if(temp_pos_direction_count1==ACCURACY)
X		if(temp_pos_direction_count1==10)
N		{
N			present_direction_flag1=1;
N			temp_peak1=present_adc_value1[1];
N			temp_pos_direction_count1=0;
N		}
N	}
N	else if(temp_peak1>present_adc_value1[0])
N	{
N		if(temp_neg_direction_count1==0)
N		{
N			pos_peak1=present_adc_value1[1];
N
N
N			for (k=2;k<BUFFER;k++)
X			for (k=2;k<6;k++)
N			{
N				if(present_adc_value1[k]>pos_peak1)
N				{
N					pos_peak1=present_adc_value1[k];
N				}
N			}
N		}
N		temp_neg_direction_count1++;
N		temp_pos_direction_count1=0;
N		if(temp_neg_direction_count1==ACCURACY)
X		if(temp_neg_direction_count1==10)
N		{
N			present_direction_flag1=0;
N			temp_peak1=present_adc_value1[1];
N			temp_neg_direction_count1=0;
N		}
N	}
N
N
N	if(present_direction_flag1!=previous_direction_flag1)
N	{
N		if(present_direction_flag1==1)
N		{
N			neg_avg_peak1=neg_peak1;
N			#ifdef PRINT_PEAKS
S			printf("neg_avg_peak1=%lu\r\n",neg_avg_peak1);
N			#endif
N
N			/*dc calculation*/
N			#ifdef CORRECT_DC
N			sum_dc1=sum_dc1+old_sum1-new_sum1;
N			old_sum1=new_sum1;
N			#endif
N
N			average_dc_value1=sum_dc1/dc_count1;
N			sum_dc1=0;
N			dc_count1=0;
N			#ifndef CORRECT_DC
S			new_sum1=0;
...
N
N			#ifdef PRINT_DC
S			printf("average_dc_value1=%lu\r\n",average_dc_value1);
N			#endif
N
N			/*ratio calculation*/
N			ratio1=calculate_ratio(average_dc_value1,pos_avg_peak1,neg_avg_peak1);
N			#ifdef PRINT_RATIOS
S			printf("ratio1=%lu\r\n",ratio1);
N			#endif
N
...
N		else
N		{
N			pos_avg_peak1=pos_peak1;
N			#ifdef PRINT_PEAKS
S			printf("pos_avg_peak1=%lu\r\n",pos_avg_peak1);
N			#endif
N		}
N
N	}
...
N		present_adc_value1[k]=present_adc_value1[k-1];
N
N	}
N
N	previous_direction_flag=present_direction_flag;
N	previous_direction_flag1=present_direction_flag1;
N
N
N	#ifdef PRINT_SPO2
S	return SPO2_buffer[SPO2_AVERAGE_LENGTH-1];
S
N	#endif
N	#ifdef PRINT_AVERAGE_SPO2
N	return SPO2_avg;
...
N	adc_value1=0;
N
N	for(k=0;k<(FILTER_LENGTH_SPO2);k++)
X	for(k=0;k<(5);k++)
N	{
N		adc_value+=raw_RED_buffer[k];
N		adc_value1+=raw_IR_buffer[k];
N
N	}
N
N	adc_value=adc_value/FILTER_LENGTH_SPO2;
X	adc_value=adc_value/5;
N	adc_value1=adc_value1/FILTER_LENGTH_SPO2;
X	adc_value1=adc_value1/5;
N}
N
N
N
