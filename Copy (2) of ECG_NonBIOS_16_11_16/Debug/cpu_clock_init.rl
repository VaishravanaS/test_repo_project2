L 1 "../src/cpu_clock_init.c"
N /******************************************************************************
N**File Name			: cpu_clock_init.c
N**File Description	:
N**Created By		:
N**Creation Date		:
...
N#endif /* _CSLR_H_ */
N
L 42 "../common_inc/cslr_pll_001.h" 2
N
N
...
N*/
N#endif
L 14 "../src/cpu_clock_init.c" 2
N
N/* Global variable, that has the system clock in terms of KHz */
NUint32 sysClk       = 100000000;
N
N/**
N *  \brief Initialize the PLL
N *
...
N * parameters are allowed and set.
N *
N * \param   pllmult [IN]    Multiplier value w.r.t to Input Clock
N * \param   plldiv  [IN]    Divisor value w.r.t to Input Clock
N *
N * \return Success, if the system clock doesn't go past 100MHz for pllmult and plldiv,
N *         failure otherwise.
N */
NPSP_Result CpuClockInit(int pllmult ,int plldiv)
N{
N    ioport Uint16* CLK_CONFIG_MSW;
N    ioport Uint16* PLL_CNTL_LSW ;
N
N    Uint32 maxClk       = 0;
N    Uint32 sysClkReqd = 0;
N
N    Uint16 Vp;
N    Uint16 Rd;
N    Uint16 Vs = 0;
...
N    Uint16 tempVal;
N
N    CLK_CONFIG_MSW  = (Uint16*)CLK_CONFIG_MSW_ADDR;
X    CLK_CONFIG_MSW  = (Uint16*)((ioport volatile unsigned*)0x1C1F);
N    PLL_CNTL_LSW    = (Uint16*)PLL_CNTL_LSW_ADDR;
X    PLL_CNTL_LSW    = (Uint16*)((ioport volatile unsigned*)0x1C20);
N
N    /* In terms of KHz, This is the required clock frequency */
N    sysClkReqd = (INPUT_CLK * (Uint32) pllmult)/(plldiv);
X    sysClkReqd = ((12000) * (Uint32) pllmult)/(plldiv);
N
N    /* In terms of KHz */
N    maxClk = (Uint32)((Uint32)SYS_CLK_MAX * 1000);
X    maxClk = (Uint32)((Uint32)(100) * 1000);
N
N    if(maxClk < sysClkReqd) return (PSP_E_INVAL_PARAM);
X    if(maxClk < sysClkReqd) return ((-5));
N
N    /* In terms of KHz, The Input to the VCO is 60 KHz */
N    Rd = INPUT_CLK/60;
X    Rd = (12000)/60;
N
N    tempVal = ( (Rd + 4) * (sysClkReqd/INPUT_CLK) );
X    tempVal = ( (Rd + 4) * (sysClkReqd/(12000)) );
N
N    /* This is to ensure that when Vp is calculated, we do not have a decimal point */
N    for(Vs=0; Vs<4; Vs++)
N    {
N        if( ((tempVal-Vs) % 4) == 0)
N        {
...
N    Vp = (tempVal - Vs - 4)/4;
N
N    /* This would be the system clock that the system would be running at */
N    sysClk = (Uint32) (INPUT_CLK * ( ( (Vp*4) +Vs+4 ) / (Rd+4) ) );
X    sysClk = (Uint32) ((12000) * ( ( (Vp*4) +Vs+4 ) / (Rd+4) ) );
N
N    /* force to BYPASS mode */
N    *CLK_CONFIG_MSW &= (SYS_PLL_BYPASS);
X    *CLK_CONFIG_MSW &= ((0xFFFE));
N
N    /* wait for BYPASS mode to be active */
N    while(*CLK_CONFIG_MSW & SYS_PLL_SEL);
X    while(*CLK_CONFIG_MSW & (0x0001));
N
N    /* PLLCNTL1 register values */
N    CSL_PLL_REGS->PLLCNTL1 &= ~CSL_PLL_PLLCNTL1_PLL_STANDBY_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL1 &= ~(0x2000u);
N    CSL_PLL_REGS->PLLCNTL1 &= ~CSL_PLL_PLLCNTL1_PLL_PWRDN_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL1 &= ~(0x1000u);
N    CSL_PLL_REGS->PLLCNTL1 |= CSL_PLL_PLLCNTL1_CLR_CNTL_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL1 |= (0x8000u);
N    CSL_PLL_REGS->PLLCNTL1 &= ~(CSL_PLL_PLLCNTL1_VP_MASK);
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL1 &= ~((0x03FFu));
N    CSL_PLL_REGS->PLLCNTL1 |= (Vp << CSL_PLL_PLLCNTL1_VP_SHIFT);
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL1 |= (Vp << (0x0000u));
N
N    /* PLLINCNTL register values */
N    CSL_PLL_REGS->PLLINCNTL &= ~CSL_PLL_PLLINCNTL_RP_BYPASS_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLINCNTL &= ~(0x8000u);
N    CSL_PLL_REGS->PLLINCNTL &= ~CSL_PLL_PLLINCNTL_VS_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLINCNTL &= ~(0x3000u);
N    CSL_PLL_REGS->PLLINCNTL |= (Vs << CSL_PLL_PLLINCNTL_VS_SHIFT);
X    ((CSL_PllRegsOvly) 0x1C20)->PLLINCNTL |= (Vs << (0x000Cu));
N    CSL_PLL_REGS->PLLINCNTL &= ~CSL_PLL_PLLINCNTL_RD_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLINCNTL &= ~(0x0FFFu);
N    CSL_PLL_REGS->PLLINCNTL |= (Rd << CSL_PLL_PLLINCNTL_RD_SHIFT);
X    ((CSL_PllRegsOvly) 0x1C20)->PLLINCNTL |= (Rd << (0x0000u));
N
N    /* PLLCNTL2 register values */
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_PLL_DIS_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x8000u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_CLK_DIS_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x4000u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_EN_VCO_DIV_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x2000u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_ENTP_SIG_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x1000u);
N    CSL_PLL_REGS->PLLCNTL2 |= CSL_PLL_PLLCNTL2_EN_LW_JITTER_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 |= (0x0800u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_PDSW_CNTL_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0200u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_PDSW_TOGDIS_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0100u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_LP_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0020u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_LW_BIAS_CURR_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0010u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_TST_LCK_MON_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0008u);
N    CSL_PLL_REGS->PLLCNTL2 |= CSL_PLL_PLLCNTL2_LNG_CLK_CNT_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 |= (0x0004u);
N    CSL_PLL_REGS->PLLCNTL2 |= CSL_PLL_PLLCNTL2_FSTRT_EN_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 |= (0x0002u);
N    CSL_PLL_REGS->PLLCNTL2 &= ~CSL_PLL_PLLCNTL2_NB_SEL_MASK;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLCNTL2 &= ~(0x0001u);
N
N    /* PLLOUTCNTL register values */
N    CSL_PLL_REGS->PLLOUTCNTL = CSL_PLL_PLLOUTCNTL_RESETVAL;
X    ((CSL_PllRegsOvly) 0x1C20)->PLLOUTCNTL = (0x0000u);
N
N    /* wait for PLL mode to be powered on */
N    while(*PLL_CNTL_LSW & 0x1000);
N
N    /* select pll */
N    *CLK_CONFIG_MSW |= SYS_PLL_SEL;
X    *CLK_CONFIG_MSW |= (0x0001);
N
N    /* wait for pll selection */
N    while (!(*CLK_CONFIG_MSW & SYS_PLL_SEL));
X    while (!(*CLK_CONFIG_MSW & (0x0001)));
N
N    return (PSP_SOK);
